...
describe("smartwallet", () => {
  ...
  it("Withdraw SOL from vault", async () => {
    ...
    // withdraw sol
    const tx = await program.methods
      .withdrawSol(new anchor.BN(1 * anchor.web3.LAMPORTS_PER_SOL))
      .accounts({
    ...
    expect(initialWalletBalance - finalWalletBalance).to.equals(
      1 * anchor.web3.LAMPORTS_PER_SOL
    );
    expect(finalOwnerBalance - initialOwnerBalance).to.equals(
      1 * anchor.web3.LAMPORTS_PER_SOL
    );
  });
